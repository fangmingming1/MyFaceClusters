...
'''
class GINFeeder(Dataset):

    '''
    Generate a sub-graph from the feature graph centered at some node,
    and now the sub-graph has a fixed depth, i.e.（那就是说、换句话说） 2
    从以某个节点为中心的特征图生成子图，
    现在子图有一个固定的深度 2
    '''
        ...
        for node in unique_nodes_list:
            ...
            for n in neighbors:
                if n in unique_nodes_list:
                    ...
                    # 利用list排序
                    edge = [unique_nodes_map[n], unique_nodes_map[node]]
                    edge.sort()
                    # 构造元组
                    edge = (edge[0], edge[1])
                    A_complex.append(edge)
                    # 利用set集合的唯一性去重，两点之间，只需要一条边就够了
                    A_complex = list(set(A_complex))
      ...
      # #  print(A_.shape)
       ...
       # print(np.asarray(unique_nodes_list))
        ...
        if self.train:
            ...
            graph_data = Data(x=feat, edge_index=A_complex, y=edge_labels, center_idx=center_idx, one_hop_idcs=one_hop_idcs, edge_label=edge_labels)
            return graph_data
            # Testing
        ...
        graph_data = Data(x=feat, edge_index=A_complex, y=edge_labels, center_idx=center_idx, one_hop_idcs=one_hop_idcs, unique_nodes_list=unique_nodes_list)
        return graph_data
