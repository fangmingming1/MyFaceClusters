...
def knns2ordered_nbrs(knns, sort=True):
    ...
    #第一种方案 切走前20个邻居和最后20个邻居拼到一起
    # nbrs_top20 = nbrs[:, 0:100]
    # nbrs_last20 = nbrs[:, 100:200]
    #
    # np.random.shuffle(nbrs_last20)
    # nbrs = np.concatenate((nbrs_top20, nbrs_last20), axis=1)
    #nbrs = nbrs[:, :10]
    # np.random.seed(1)
    nbrs = nbrs[:, :199]
    index = np.array(range(nbrs.shape[0])).reshape(-1,1)
    nbrs = np.concatenate((index, nbrs), axis=1)
    # ylen = nbrs.shape[0]
    # for i in range(ylen):
    #     if i != nbrs[i][0]:
    #         print(i)
    #         nbrs[i][0] = i
        #np.random.shuffle(nbrs[i][1:])
    return dists, nbrs
...
'''
def knns2ordered_nbrs_forYTB(knns, sort=True):
    knns = np.array(knns)
    nbrs = []
    nbrs = knns[:, 0]
    nbrs = np.array(nbrs)
...
def intdict2ndarray(d, default_val=-1):
    arr = np.zeros(len(d), dtype=int) + default_val
    for k, v in d.items():
        arr[k] = v
    return arr
